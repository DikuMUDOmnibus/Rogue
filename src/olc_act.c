...
//  argument=one_argument(argument, phrase);
  argument=strcpy(phrase,argument);
...
/** Function: gedit_create
  * Descr   : Creates a new, empty guild if room is available in the
array.
...
GEDIT( gedit_show )
{
  ...
  sprintf(buf, "{mMortal Leader Rights\n\r--------------------{x\n\r");
  add_buf(buffer, buf);
...
/** Function: gedit_whoname
  ...
  * Returns : True/False if modified.  * Syntax  : whoname new_who_name
  * Written : v1.0 3/98
  * Author  : Gary McNickle <gary@dharvest.com>
  */
GEDIT( gedit_whoname )
...
GEDIT( gedit_ml )
{
  ...
  if ( argument[0] == '\0' )
  {
    send_to_char( "Syntax:  ml TRUE|FALSE TRUE|FALSE TRUE|FALSE
TRUE|FALSE\n\r", ch );
    return FALSE;
  }
