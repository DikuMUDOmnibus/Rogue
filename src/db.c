...
char			ann_buf		[2*MAX_INPUT_LENGTH];
char			pueblo_buf	[2*MAX_INPUT_LENGTH];
...
//	pMobIndex->long_descr[0]	= pMobIndex->long_descr[0]
...
void load_rooms( FILE *fp )
{
	...
	for ( ; ; )
	{
	   ...
	   else if ( letter == 'C') /* clan */
	   {
		...
		switch ( locks )
		{
                    case 1: pexit->rs_flags  = EX_ISDOOR;
			break;
                    case 2: pexit->rs_flags  = EX_ISDOOR | EX_PICKPROOF; 
			break;
		    case 3: pexit->rs_flags  = EX_ISDOOR | EX_HIDDEN;
			break;
                    case 4: pexit->rs_flags  = EX_ISDOOR |EX_PICKPROOF | 
				EX_HIDDEN;
			break;
                    case 5: pexit->rs_flags  = EX_ISDOOR | EX_NOPASS;
			break;
                    case 6: pexit->rs_flags  = EX_ISDOOR | EX_PICKPROOF | 
				EX_NOPASS;
			break;
                    case 7: pexit->rs_flags  = EX_ISDOOR | EX_HIDDEN | 
				EX_NOPASS;
			break;
                    case 8: pexit->rs_flags  = EX_ISDOOR | EX_PICKPROOF | 
				EX_HIDDEN | EX_NOPASS;
			break;
		case 9: pexit->rs_flags = EX_HIDDEN; break;
		}
	...
	else if (letter == 'I')
	{
		...
		{
		    bug("Load_rooms: duplicate p_image.",0);
		    exit(1);
		}
...
void do_memory( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf, "Mobs    %5d", top_mob_index); 
    send_to_char( buf, ch );
