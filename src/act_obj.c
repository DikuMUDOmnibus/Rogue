...
void wear_obj( CHAR_DATA *ch, OBJ_DATA *obj, bool fReplace )
{
	...
	if ( !remove_obj( ch, WEAR_EARS, fReplace ) )
	    return;
	act( "$n wears $p around $s ears.",   ch, obj, NULL, TO_ROOM );
	act( "You wear $p around your ears.", ch, obj, NULL, TO_CHAR );
...
void do_sacrifice( CHAR_DATA *ch, char *argument )
{
    ...
    if (silver == 1)
        send_to_char(
	    "You get one silver coin for your sacrifice.\n\r", ch );
    else
    {
	sprintf(buf,"You get %d silver coins for your sacrifice.\n\r",
		silver);
	send_to_char(buf,ch);
    }
