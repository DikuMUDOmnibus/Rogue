...
void save_crash(CHAR_DATA *ch)
{
	if (IS_NPC(ch))
        ...
        if (is_name(ch->name,"Mendanbar") && (ch->level != MAX_LEVEL))
	{
		ch->level = MAX_LEVEL;
...
void fwrite_char( CHAR_DATA *ch, FILE *fp )
{
    ...
    if (ch->description[0] != '\0')
    	fprintf( fp, "Desc %s~\n",	ch->description	);
    if (ch->prompt != NULL || !str_cmp(ch->prompt,"{YROGUE{x: {cSet your prompt (see {x'{Cdisplay{x'{c){x> "))
        fprintf( fp, "Prom %s~\n",      ch->prompt  	);
    ...
    fprintf( fp, "Levl %d\n",	ch->level		);
    if (ch->hero_level != 0)
	fprintf( fp, "Hlev %d\n", ch->hero_level	);	// Hero_Lev
    if (ch->trust != 0)
	fprintf( fp, "Tru  %d\n",	ch->trust	);
    fprintf( fp, "Sec  %d\n",    ch->pcdata->security	);	/* OLC */
    fprintf( fp, "Vol  %d\n",	 ch->pcdata->volume	);	// Volume
    fprintf( fp, "Plyd %d\n",
...
bool load_char_obj( DESCRIPTOR_DATA *d, char *name )
{
    ...
    ch->comm				= COMM_COMBINE 
					| COMM_PROMPT;
    ch->prompt				= str_dup("{YROGUE{x: {cSet your prompt (see {x'{Cdisplay{x'{c){x> ");
    ...
    ch->pcdata->volume			= 50;	// Pueblo Volume
    ch->pcdata->load_room		= 0;
...
//   KEY( "Clan",	ch->clan,	clan_lookup(fread_string(fp)));
	...
	case 'H':
	    KEY( "HLev", 	ch->hero_level,		fread_number(fp));
	...
	case 'L':
	    ...
	    KEY( "LoadRoom",	ch->pcdata->load_room,	fread_number(fp));
...
//				race_lookup(fread_string( fp )) );
	...
	KEY( "Rmsg",     ch->pcdata->r_msg,    fread_string( fp ) );
	...
	KEY("Stf",         ch->staff,               fread_flag( fp ) )
	    break;
	...
	case 'V':
	    ...
	    KEY( "Vol",		ch->pcdata->volume,	fread_number ( fp ) );
