...
void gain_exp( CHAR_DATA *ch, int gain )
{
	...
	return;
    if (ch->hero_level == MAX_HERO_LEVEL)
	return;
...
(IS_REAL_HERO(ch) ? ((ch->level+ch->hero_level)+1) : (ch->level+1)))
	...
	send_to_char( "You raise a level!!  ", ch );
		if (IS_REAL_HERO(ch)) {
			ch->hero_level += 1;
			info(ch, 0, "{y[{bINFO{y] %s has gained Hero Level %d!{x\n\r",ch->name,ch->hero_level);
			sprintf(buf, "%s gained hero level %d!",ch->name,ch->hero_level);
		} else {
		ch->level += 1;
		info(ch, 0, "{y[{bINFO{y] %s has gained Level %d!{x\n\r",ch->name,ch->level);
		sprintf(buf,"%s gained level %d",ch->name,ch->level);
		}
		log_string(buf);
		sprintf(buf,"$N has attained level %d!",IS_REAL_HERO(ch) ? ch->hero_level : ch->level);
