...
void do_rmsg(CHAR_DATA *ch, char *argument)
{
    char buf[MSL];
...
void do_broadcast(CHAR_DATA *ch, char *argument )
{
    ...
    if ( argument[0] == '\0' )
    {
	send_to_char("Yes, broadcast, fine, broadcast we must, but WHAT???\r\n",ch);
	return;
    }

    sprintf(buf, "{y[{bINFO{y] %s{w: %s{x\r\n",ch->name,argument);
    printf_to_char(ch,"{y[{bINFO{y] ->{w %s{x\r\n",argument);
...
void do_load(CHAR_DATA *ch, char *argument )
{
   char arg[MAX_INPUT_LENGTH];
    ...
    if (!str_cmp(arg,"mob") || !str_cmp(arg,"m"))
    {
...
void do_mset( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "    security loadroom\n\r", ch );
	return;
    }
    ...
    if (!str_cmp(arg2, "loadroom")) /* LOADROOMS */
    {
	if (IS_NPC(victim)) {
	...
	if (!str_cmp(arg3, "off")) {
		REMOVE_BIT(victim->act, PLR_LOADROOM);
		send_to_char("Loadroom off.\n\r",ch);
		return;
	}
	if (value < 1) {
		send_to_char("Must be higher than 1!\n\r",ch);
		return;
	}
	...
	victim->pcdata->load_room = value;
	SET_BIT(victim->act, PLR_LOADROOM);
	send_to_char("Loadroom set!\n\r",ch);
	return;
    }
...
strcat(buf2,
"Num Race      Name         State          Idl Login@  Site\r\n");
    strcat( buf2,
"--- --------- ------------ -------------- --- ------- ------------------------\r\n");
    for ( d = descriptor_list; d; d = d->next )
    {
	strcat(buf2,d->connected == CON_PLAYING ? "{x" : "{G");
...
case CON_GET_NEW_RACE:          st = "Get Race      ";    break;
case CON_GET_NEW_SEX:           st = "Get Sex       ";    break;
case CON_GET_NEW_CLASS:        	st = "Get Class     ";    break;
case CON_GET_ALIGNMENT:		st = "Get Align     ";	break;
case CON_DEFAULT_CHOICE:	st = "Choose Cust   ";	break;
case CON_GEN_GROUPS:		st = "Customization ";	break;
case CON_PICK_WEAPON:		st = "Choose Weap   ";	break;
...
case CON_CHOOSE_TERM:		st = "Choose Term   "; break;
           ...
           if ( (vch->timer >= 0) && !IS_STAFF(vch))
              sprintf( idle, "%3d", vch->timer );
	  else sprintf(idle, "   " );
           sprintf( buf, "%3d [%3d %3s] %-12s %s %3s %7s %-32.32s\n\r",
              d->descriptor,
	vch->level,
	vch->race < MAX_PC_RACE ? pc_race_table[vch->race].who_name : "NULL",
...
void do_copyover (CHAR_DATA *ch, char * argument)
{
	...
	execl(EXE_FILE, "rom", buf, "rogue", buf2, (char *) NULL);
