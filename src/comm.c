...
void bust_a_prompt( CHAR_DATA *ch )
{
    ...
    if( !str || str[0] == '\0')
    { /*
        sprintf( buf, "{c%d(%d)hp %dm %dmv>{x %s",
	    ch->hit, ch->max_hit, ch->mana, ch->move, ch->prefix );
	send_to_char( buf, ch );
*/
	send_to_char("{YROGUE{x: {cSet your prompt (see {x'{Cdisplay{x'{c){W>{x",ch);
	return;
    }
   if (ch->invis_level > 0) {
	sprintf(buf, "{gw%d{x ",ch->invis_level);
	send_to_char(buf,ch);
	}
   if (ch->incog_level > 0) {
	sprintf(buf, "{gi%d{x ",ch->incog_level);
	send_to_char(buf,ch);
	}
	...
	if ((victim = ch->fighting) != NULL)
           {
            if (victim->max_hit > 0)
                ...
                percent = -1;	
              if(percent >= 65)
                 sprintf(buf2,"{r%s {w[{g%d%%{w]{x",victim->name,percent);
              else if(percent >= 25 && percent < 65)
                 sprintf(buf2,"{r%s {w[{Y%d%%{w]{x",victim->name,percent);
              else
                 sprintf(buf2,"{r%s {w[{r%d%%{w]{x",victim->name,percent);
              i = buf2;
           } else
              i = "";         
           break;
...
case CON_DEFAULT_CHOICE:
	...
	write_to_buffer( d, 
    "Welcome to ROGUE 2.0.  Feed the mobs at your own risk!\n\r",
	    0 );
	ch->next	= char_list;
	char_list	= ch;
	d->connected	= CON_PLAYING;
	...
	sprintf(pueblo_buf,"<img xch_sound=play xch_volume=%d src=\"%s%s\">"
		"<img src=\"%s%s\">", ch->pcdata->volume,PUEBLO_DIR,PUEBLO_START, PUEBLO_DIR, PUEBLO_S_IMG);
	...
	else if (IS_SET(ch->act, PLR_LOADROOM))
	{
	    char_to_room(ch, get_room_index(ch->pcdata->load_room));
	}
...
*/	else if ( IS_IMMORTAL(ch) )
	{
	    char_to_room( ch, get_room_index( ROOM_VNUM_STAFF));
	}
	else
	{
	    char_to_room(ch,get_room_index(pc_race_table[ch->race].startroom));
	}

	act( "$n has entered the game.", ch, NULL, NULL, TO_ROOM );
	do_function(ch, &do_look, "auto" );
	info( ch, 0, "{y[{bINFO{y] %s has entered the game!{x\n\r", ch->name );
...
bool check_parse_name( char *name )
{
	...
	"all auto immortal self someone something the you demise balance circle honor none"
	"unlinked"))
    {
	return FALSE;
    }
