	    ...
	    bool show) 
{
     ...
     */
/*	Lets try combat in the skinnies...
...
*/
	...
	if ( !IS_NPC(victim) )
	{
	     ...
	     */
//  Removed By Mendanbar - Death sucks!
	}
	if (!IS_NPC(victim))
	info( NULL, 0, "{y[{bINFO{y]{x %s killed by %s at %s.{x\n\r",victim->name,
                (IS_NPC(ch) ? ch->short_descr : ch->name),
                ch->in_room->name);
...
void dam_message( CHAR_DATA *ch, CHAR_DATA *victim,int dam,int dt,bool immune )
{
	...
	else
	{
	    ...
	    {
        sprintf( buf1, "$n's %s %s $N%c {w({y%d{w){x",  attack, vp,punct,dam );
        sprintf( buf2, "Your %s %s $N%c {w({y%d{w){x",attack,vp,punct,dam);
        sprintf( buf3, "$n's %s %s you%c {w({y%d{w){x",attack,vp,punct,dam);
      		}
	    else
	    {
