...
void extract_char( CHAR_DATA *ch, bool fPull )
{
    ...
    */
	...
	if (!IS_SET(obj->extra_flags, ITEM_NO_LOSE))
	extract_obj( obj );
    }
...
bool can_see_ex_hidden( CHAR_DATA *ch, EXIT_DATA *pexit)
{
...
char *staff_bit_name(int staff_flags)
{
    static char buf[512];
    buf[0] = '\0';
	if (staff_flags & STAFF_GEN	) strcat(buf, " general");
	if (staff_flags & STAFF_ADMIN	) strcat(buf, " admin");
	if (staff_flags & STAFF_SECURITY) strcat(buf, " security");
	if (staff_flags & STAFF_GAME	) strcat(buf, " game");
	if (staff_flags & STAFF_CHAR	) strcat(buf, " character");
	if (staff_flags & STAFF_CLANS	) strcat(buf, " clans");
	if (staff_flags & STAFF_OLC	) strcat(buf, " olc");
	if (staff_flags & STAFF_OLCADMIN) strcat(buf, " olcadmin");
	if (staff_flags & STAFF_SOCIALS) strcat(buf, " socials");
	if (staff_flags & STAFF_HELP	) strcat(buf, " help");
	if (staff_flags & STAFF_SCRIPT	) strcat(buf, " scripts");
