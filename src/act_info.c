...
void show_char_to_char_0( CHAR_DATA *victim, CHAR_DATA *ch )
{
    ...
    if ( IS_AFFECTED(victim, AFF_CHARM)       ) strcat( buf, " {m(charmed){x");
    if ( IS_AFFECTED(victim, AFF_PASS_DOOR)   ) strcat( buf," {c(translucent){x");
    if ( IS_AFFECTED(victim, AFF_FAERIE_FIRE) ) strcat( buf, " {M(PinkAura{x)");
    if ( IS_EVIL(victim)
    &&   IS_AFFECTED(ch, AFF_DETECT_EVIL)    ) strcat( buf, " {R(RedAura){x");
    if ( IS_GOOD(victim)
    &&   IS_AFFECTED(ch, AFF_DETECT_GOOD)     ) strcat( buf, " {Y(GoldenAura){x");
    if ( IS_AFFECTED(victim, AFF_SANCTUARY)   ) strcat( buf, " {W(glowing){x");
    if ( IS_SET(victim->comm,COMM_AFK     )   ) strcat( buf, " {c[AFK]{x");
    if ( !IS_NPC(victim) && IS_SET(victim->act, PLR_KILLER ) )
                        strcat( buf, " {R(KILLER){x");
    if ( !IS_NPC(victim) && IS_SET(victim->act, PLR_THIEF  ) )
                        strcat( buf, " {B(THIEF){x");
...
void do_autolist(CHAR_DATA *ch, char *argument)
{
   ...
   send_to_char("autodamage     ",ch);
    ...
    send_to_char("automapper     ",ch);
	send_to_char(IS_SET(ch->act,PLR_AUTOMAP) ? "ON\n\r" : "OFF\n\r",ch);
...
void do_automap(CHAR_DATA *ch, char *argument)
  {
	...
	if (IS_SET(ch->act,PLR_AUTOMAP)) {
		send_to_char("AutoMap is now OFF.\n\r",ch);
		REMOVE_BIT(ch->act,PLR_AUTOMAP);
	} else {
		send_to_char("AutoMap is now ON.\n\r",ch);
...
void do_display(CHAR_DATA *ch, char *argument)
{
	...
	{ "Colorized Stock Rogue", "{R%h{rhp {M%m{mm {G%v{gmv{c>{x "},
	{ "Standard", "{R%h{rhp {M%m{mm {G%v{gmv {yexp: %X{x> "},
	{ "Full Featured", "{R%h{w({R%H{w){Whitp {M%m{w({M%M{w){Wmana {G%v{w({G%V{w){Wmove{x> %b{x "},
	{ "Non Magic Featured", "{R%h{w({R%H{w){Whitp {G%v{w({G%V{w){Wmove{x> %b "},
	{ "\n", "\n"}
	};
...
void do_look( CHAR_DATA *ch, char *argument )
{
    ...
    {
	/* 'look' or 'look auto' */
	if ((!IS_NULLSTR(ch->in_room->p_image)) && (IS_SET(ch->act,PLR_PUEBLO))) {
		send_to_char("</xch_mudtext><img xch_mode=html>",ch);
		sprintf(pueblo_buf,"<img src=\"%s%s\">",PUEBLO_DIR,ch->in_room->p_image);
...
void do_exits( CHAR_DATA *ch, char *argument )
{
    ...
    bool found;
    bool fAuto;
    const char *dir_auto_name[] = {"n","e","s","w","u","d"};
    int door;
		...
		)
	{
	    ...
	    if (fAuto)
	    {
sprintf( buf + strlen(buf), "{%s%s {x",
IS_SET(pexit->exit_info, EX_HIDDEN) ? "g" : "c",
!IS_SET(pexit->exit_info, EX_CLOSED) ? capitalize(dir_auto_name[door]) : dir_auto_name[door] );
	    }
	    else
	    {
...
void do_worth( CHAR_DATA *ch, char *argument )
{
	...
	(IS_REAL_HERO(ch) ? ((ch->level+ch->hero_level)+1) : (ch->level+1))*exp_per_level(ch,ch->pcdata->points) - ch->exp);
...
void do_who( CHAR_DATA *ch, char *argument )
{
	...
	*buf = '\0';
	...
	default: break;
            {
	case MAX_LEVEL - 0 : class = "   Coder   ";     break;
	case MAX_LEVEL - 1 : class = "   Admin   ";     break;
	case MAX_LEVEL - 2 : class = " Associate ";     break;
	case MAX_LEVEL - 3 : class = "Security/PR";     break;
	case MAX_LEVEL - 4 : class = "Sr. Builder";     break;
	case MAX_LEVEL - 5 : class = "  Questor  ";     break;
	case MAX_LEVEL - 6 : class = "Terraformer";     break;
	case MAX_LEVEL - 7 : class = " WebMaster ";     break;
	...
	*buf = '\0';
        ...
        if (IS_STAFF(wch)) {
	        sprintf(buf, "{y %s %s%s{x",
        	        class, wch->name, wch->pcdata->title);
	staff++;
	} else if (IS_REAL_HERO(wch)) {
		sprintf(buf, "{x[%3d {YHERO{x] %s%s{x",
			wch->hero_level, wch->name, wch->pcdata->title);
	players++;
	} else {
...
/*
char *relation_colors = "xyr";

int GetRelation(CHAR_DATA *target) {
	CHAR_DATA *ch;

	if (IS_STAFF(ch) || IS_STAFF(target))
		return RELATION_FRIEND;
	if (IS_TRAITOR(ch) ^ IS_TRAITOR(target))
